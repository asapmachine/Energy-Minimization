...
def transfer_and_split_data(csv_a_path, source_csvs, matched_output, unmatched_output):
    ...
    for file in source_csvs:
        try:
            df_temp = pd.read_csv(file, usecols=['filename', 'Df'])
    ...
    combined_source = combined_source.dropna(subset=['Df'])
    ...
    # 创建一个布尔掩码：如果 'Df_已匹配' 是空值，则为 True
    unmatched_mask = result_df['Df_y'].isna()
...
if __name__ == "__main__":
    # 请根据你实际的文件路径进行替换
    file_a = r'E:\CODE\cif2des\clean_data\merged_features.csv'
    # 数据源表 B, C, D 的路径列表
    files_bcd = [r'E:\CODE\cif2des\clean_data\goal_data\RAC_and_geom_1inor_1edge.csv', r'E:\CODE\cif2des\clean_data\goal_data\RAC_and_geom_1inor_1org_1edge.csv', r'clean_data/goal_data/RAC_and_geom_2inor_1edge.csv']
    # 成功匹配的最终文件（即你需要的 train_data.csv）
    matched_target_file = r'E:\CODE\cif2des\clean_data\goal_data\train_data.csv'
    
    # 没匹配上的 30 个文件单独存放在这里，供人工排查
    unmatched_error_file = r'E:\CODE\cif2des\clean_data\goal_data\unmatched_30_error.csv'
