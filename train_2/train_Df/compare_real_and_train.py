...
def main():
    print("=============== 1. 加载数据与严格对齐 ===============")
    csv_path = r'E:\CODE\cif2des\train_2\train_Df\clean_data\A_final_cleaned_data_racs.csv'
    model_path = 'catboost_mof_df_predictor_best_with_RACs.cbm' 
    
    df = pd.read_csv(csv_path)

    geo_features = ['Di', 'Df_x', 'Dif', 'rho', 'VSA', 'GSA', 'VPOV', 'GPOV', 'POAV_vol_frac', 'GPOAV', 'POAV']
    rac_features = [
    ...
    print("\n" + "🔥" * 25)
    print("     物理形变基准 vs 高维机器学习预测 对比报告 (Df)")
    print("🔥" * 25)
    ...
    ax1.set_title("未优化结构 (Baseline: $Df_{unopt}$ vs $Df_{opt}$)", fontsize=14, pad=15)
    ax1.set_xlabel("真实限制孔径 (Optimized Df, Å)", fontsize=12)
    ax1.set_ylabel("未优化限制孔径 (Unoptimized Df, Å)", fontsize=12)
    ...
    ax2.set_title("高维机器学习预测 (ML+RACs: $Df_{pred}$ vs $Df_{opt}$)", fontsize=14, pad=15)
    ax2.set_xlabel("真实限制孔径 (Optimized Df, Å)", fontsize=12)
    ax2.set_ylabel("预测限制孔径 (Predicted Df, Å)", fontsize=12)
    ...
    out_dir = r'E:\CODE\cif2des\train_2\train_Df\final_result'
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    save_path = f"{out_dir}/Df_Baseline_vs_ML_Comparison_with_RACs.png"
