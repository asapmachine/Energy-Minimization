...
def main():
    print("=============== 1. 加载数据与严格对齐 ===============")
    csv_path = r'E:\CODE\cif2des\train_2\train_Dif\clean_data\A_final_cleaned_data.csv'
    model_path = 'catboost_mof_dif_predictor_best_with_RACs.cbm' 
    ...
    geo_features = ['Di', 'Df', 'Dif_x', 'rho', 'VSA', 'GSA', 'VPOV', 'GPOV', 'POAV_vol_frac', 'GPOAV', 'POAV']
    rac_features = [
    ...
    print("\n" + "🔥" * 25)
    print("     物理形变基准 vs 高维机器学习预测 对比报告 (Dif)")
    print("🔥" * 25)
    
    print("\n【未优化基准 (Baseline - Dif_x)】:")
    ...
    ax1.set_title("未优化结构 (Baseline: $Dif_{unopt}$ vs $Dif_{opt}$)", fontsize=14, pad=15)
    ax1.set_xlabel("真实扩散孔径 (Optimized Dif, Å)", fontsize=12)
    ax1.set_ylabel("未优化扩散孔径 (Unoptimized Dif, Å)", fontsize=12)
    ...
    ax2.set_title("高维机器学习预测 (ML+RACs: $Dif_{pred}$ vs $Dif_{opt}$)", fontsize=14, pad=15)
    ax2.set_xlabel("真实扩散孔径 (Optimized Dif, Å)", fontsize=12)
    ax2.set_ylabel("预测扩散孔径 (Predicted Dif, Å)", fontsize=12)
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    save_path = f"{out_dir}/Dif_Baseline_vs_ML_Comparison_with_RACs.png"
