...
def main():
    ...
    # ---------------------------------------------------------
    # 对手 B：CatBoost 高维增强模型 (融合 RACs)
    # ---------------------------------------------------------
    print("正在从硬盘加载已训练好的高维增强模型...")
    model = cb.CatBoostRegressor()
    try:
        model.load_model('catboost_mof_di_predictor_best_with_RACs.cbm')
    except Exception as e:
        print(f"❌ 模型加载失败，请确保 'catboost_mof_di_predictor_best_with_RACs.cbm' 在当前目录下！")
        return
    ...
    # ==========================================
    print("\n" + "🔥" * 25)
    print("   物理形变基准 vs 高维机器学习预测 对比报告 (Di)")
    print("🔥" * 25)
    ...
    print("\n【CatBoost 模型预测 (Machine Learning + RACs)】:")
    ...
    ax1.set_xlabel("真实最大包含球直径 (Optimized Di, Å)", fontsize=12)
    ax1.set_ylabel("未优化最大包含球直径 (Unoptimized Di, Å)", fontsize=12)
    ...
    ax2.set_title("高维机器学习预测 (ML+RACs: $Di_{pred}$ vs $Di_{opt}$)", fontsize=14, pad=15)
    ax2.set_xlabel("真实最大包含球直径 (Optimized Di, Å)", fontsize=12)
    ax2.set_ylabel("预测最大包含球直径 (Predicted Di, Å)", fontsize=12)
    ...
    save_filename = os.path.join(out_dir, 'Di_Baseline_vs_ML_with_RACs.png')
    plt.savefig(save_filename, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"✅ 大功告成！完美对比图已保存为：{save_filename}")
