...
def main():
    print("=============== 1. 加载数据与严格对齐 ===============")
    # ⚠️ 已经根据你之前的脚本自动匹配了 Dif 的正确路径！
    csv_path = r'E:\CODE\cif2des\train\train_Dif\clean_data\train_data_Dif_final.csv'
    model_path = 'catboost_mof_dif_predictor_best.cbm' # 你的 Dif 模型路径
    ...
    # ⚠️ 特征列表：确保把 Dif 改为 Dif_x，同时把 Di 还原
    ...
    # ==========================================
    print("\n" + "🔥" * 25)
    print("     物理形变基准 vs 机器学习预测 对比报告 (Dif)")
    print("🔥" * 25)
    ...
    # 标题更新
    ax2.set_title("机器学习预测 (CatBoost: $Dif_{pred}$ vs $Dif_{opt}$)", fontsize=14, pad=15)
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    save_path = f"{out_dir}/Dif_Baseline_vs_ML_Comparison.png"
