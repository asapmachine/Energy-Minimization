...
def merge_and_compare_POAV(file_a, file_b, matched_output, unmatched_output):
    ...
    if 'POAV' in df_a.columns:
        df_a = df_a.rename(columns={'POAV': 'POAV_x'})
    ...
    df_b = df_b.rename(columns={'name': 'cif_name', 'POAV': 'POAV_y'})
    ...
    # ================= 4. 数据分流 =================
    unmatched_mask = result_df['POAV_y'].isna()
    unmatched_df = result_df[unmatched_mask]
    matched_df = result_df[~unmatched_mask]
    
    print("\n>>> 合并结果核对：")
    print(f"    - 匹配成功并拿到 POAV_y 的行数: {len(matched_df)}")
...
if __name__ == "__main__":
    file_a = r'E:\CODE\cif2des\train_POAV\clean_data\train_data_poav_updated.csv'
    file_b = r'E:\CODE\cif2des\calc\clean_data\goal_data\final_ABC_features_optimal.csv'
    matched_target = r'E:\CODE\cif2des\train_POAV\clean_data\train_data_POAV.csv'
    unmatched_error = r'E:\CODE\cif2des\train_POAV\clean_data\unmatched_error.csv'
    
    merge_and_compare_POAV(file_a, file_b, matched_target, unmatched_error)