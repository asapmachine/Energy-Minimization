...
# 2. 确定预测目标 (Y)
# 【核心改动 1】：使用 np.log1p 对跨度极大的 GSA 进行对数压缩
# 真实物理空间 (0~8000) -> 对数空间 (0~8.98)
...
plt.xlabel("真实值 (优化后 GSA, $m^2/g$)")
plt.ylabel("预测值 (优化后 GSA, $m^2/g$)")
...
# 4. 模型持久化入库
final_model.save_model('catboost_mof_GSA_predictor_best.cbm')
