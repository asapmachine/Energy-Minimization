...
def transfer_and_split_data(csv_a_path, source_csvs, matched_output, unmatched_output):
    ...
    for file in source_csvs:
        try:
            df_temp = pd.read_csv(file, usecols=['filename', 'GSA'])
    ...
    combined_source = combined_source.dropna(subset=['GSA'])
    ...
    unmatched_mask = result_df['GSA_y'].isna()
...
if __name__ == "__main__":
    # 请根据你实际的文件路径进行替换
    file_a = r'E:\CODE\cif2des\train_GSA\clean_data\train_data.csv'
    # 数据源表 B, C, D 的路径列表
    ...
    matched_target_file = r'E:\CODE\cif2des\train_GSA\clean_data\train_data_GSA.csv'
    
    # 没匹配上的 30 个文件单独存放在这里，供人工排查
    unmatched_error_file = r'E:\CODE\cif2des\train_GSA\clean_data\unmatched_30_error.csv'
