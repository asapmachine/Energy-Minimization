...
def transfer_and_split_data(csv_a_path, source_csvs, matched_output, unmatched_output):
    ...
    for file in source_csvs:
        try:
            df_temp = pd.read_csv(file, usecols=['filename', 'Di'])
    ...
    combined_source = combined_source.dropna(subset=['Di'])
    ...
    unmatched_mask = result_df['Di_y'].isna()
...
if __name__ == "__main__":
    ...
    matched_target_file = r'E:\CODE\cif2des\train_Di\clean_data\train_data_Di.csv'
    
    # 没匹配上的 30 个文件单独存放在这里，供人工排查
    unmatched_error_file = r'E:\CODE\cif2des\train_Di\clean_data\unmatched_30_error.csv'
